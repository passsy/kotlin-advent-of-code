...
object Day20_ParticleSwarm {
    ...
    private val inputRegex = "p=<(.*)>, v=<(.*)>, a=<(.*)>".toRegex()
    fun toParticle(index: Int, input: String): Particle = inputRegex
            .find(input)!!.groupValues.drop(1)
            .map { it.parseVector() }
            .let { (p, v, a) -> Particle(index, p, v, a) }

    private fun String.parseVector() = split(",").map(String::toLong).let { (x, y, z) -> Vector(x, y, z) }

    data class Particle(val id: Int, val position: Vector, val velocity: Vector, val acceleration: Vector) {
        fun tick() = Particle(id, position + velocity + acceleration, velocity + acceleration, acceleration)
    }

    data class Vector(val x: Long, val y: Long, val z: Long) {
        fun distance(origin: Vector) = abs(origin.x - x) + abs(origin.y - y) + abs(origin.z - z)
    ...
    fun leftoversAfterCollisions(all: List<Particle>): List<Particle> {
        ...
        repeat(100) {
            ...
            particles -= particles.groupBy { it.position }.values.filter { it.size > 1 }.flatten()
        }
        return particles
    }
}