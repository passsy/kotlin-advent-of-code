...
class BuildCircuitTest : Spek({
    ...
    describe("Circuit") {
        test("get") {
            val circuit = Circuit()
            circuit.instructions.add(Instruction("a", StaticSignal(123)))
        ...
        test("and") {
            val circuit = Circuit()
            circuit.instructions.add(Instruction("a", StaticSignal(123)))
            circuit.instructions.add(Instruction("b", StaticSignal(67)))
            circuit.instructions.add(Instruction("c", Operation(
        ...
        test("shift left") {
            val circuit = Circuit()
            circuit.instructions.add(Instruction("a", StaticSignal(123)))
            circuit.instructions.add(Instruction("b", StaticSignal(2)))
            circuit.instructions.add(Instruction("c", Operation(
        ...
        test("not") {
            val circuit = Circuit()
            circuit.instructions.add(Instruction("a", StaticSignal(123)))
            circuit.instructions.add(Instruction("c", Not(Signal("a", circuit))))
