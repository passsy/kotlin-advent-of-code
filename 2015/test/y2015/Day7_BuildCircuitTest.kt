...
class Day7_BuildCircuitTest {
    ...
    fun `calculate - sample`() {
        val input = resourceFileText("2015/day7_sample").lines()
    ...
    @Test
    fun `small`() {
        val input = resourceFileText("2015/day7-1").lines()
        val circuit = assembleCircuit(input)
    ...
    fun `Circuit - shift left`() {
        ...
        circuit.instructions.add(CircuitInstruction("c", Operation(
                Signal("a", circuit), Signal("b", circuit),
    ...
    fun `Circuit - not `() {
        val circuit = Circuit()
        circuit.instructions.add(CircuitInstruction("a", StaticSignal(123)))
        circuit.instructions.add(
                CircuitInstruction("c", Not(Signal("a", circuit))))
    ...
    fun `Operation toString()`() {
        val op = Operation(StaticSignal(1), StaticSignal(2),
                OperationType.AND)
