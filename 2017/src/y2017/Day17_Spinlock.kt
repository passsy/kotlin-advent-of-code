...
object Day17_Spinlock {
    ...
    fun shortCircuitSpinLock(steps: Int, rounds: Int, numberAfter: Int): Int {
        var cursor = 1
        val state = BigList(listOf(0))

        (1..rounds).forEach { num ->
            cursor = (cursor + 1 + steps) % state.size
            state.add(cursor, num)

            if (num % 10000 == 0) println("${num / rounds.toDouble() * 100}%")
        }

        val index = state.indexOf(numberAfter)
