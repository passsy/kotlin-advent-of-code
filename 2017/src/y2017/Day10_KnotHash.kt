...
import com.pascalwelsch.aoc.toHex

fun main(args: Array<String>) {
    Day10_KnotHash.part1()
    Day10_KnotHash.part2()
}

object Day10_KnotHash {
    //--- Day 10: Knot Hash ---
    //
    ...
    fun knotHashSequence(lengths: List<Int>, listSize: Int = 255, rounds: Int = 64): List<Int> {
        var elements = (0..listSize).toList()
        var skipSize = 0
        var currentPosition = 0
        for (round in 1..rounds) {
            lengths.forEach { length ->
                elements = elements.knot(currentPosition, length)
                currentPosition = (currentPosition + length + skipSize) % (listSize + 1)
                skipSize++
            }
        }
        return elements
    ...
    fun List<Int>.knot(start: Int, length: Int): List<Int> {
        require(length <= size)
        ...
        if (start + length > size) {
            ...
            return begin + unwrapped.subList(overhang, size)
        } else {
            return unwrapped.subList(0, size)
        }
    }
    ...
    fun denseHash(sparseHash: List<Int>): List<Int> {
        return sparseHash.chunked(16).map { it.reduce { acc, other -> acc xor other } }
    }

    fun createInputSequence(input: String): List<Int> {
        val lengths = input.map {
            val unsigned = it.toInt()
            require(unsigned < 256) { "char ($unsigned) is not in range" }
            unsigned
        }
        return lengths + listOf(17, 31, 73, 47, 23)
    }
}
